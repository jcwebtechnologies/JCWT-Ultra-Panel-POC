...
func CreateSystemUser(username, webRootBase string) error {
	...
	cmd = exec.Command("sudo", "mkdir", "-p", logsDir)
	if output, err := cmd.CombinedOutput(); err != nil {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("create php logs dir: %s", string(output))
	}
	...
	// Set logs directory permissions (www-data needs write access for nginx)
